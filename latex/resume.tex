...
\item{Member of Cloud Platform Group providing a developer centric cloud platform abstraction}
  \begin{itemize}
    \item{200+ Clusters OpenStack/GKE, 4500+ Workload VMs}
    \item{Helm Chart authoring and maintenance}
    \item{Documentation and Runbook creation for PagerDuty automation}
    \item{Kubernetes Upgrades and API Deprecation Preparedness}
    \item{Automated deployment of regional Kubernetes platform (IaaS) using Cue and ArgoCD}
    \item{Custom Kubernetes Operator creation/maintenance}
    \item{Production incident response and issue debugging}
    \item{Code Review and feature development}
    \item{Istio Service Mesh 1.10.x -> 1.15.7 platform upgrade}
  \end{itemize}
\item{Member of Observability team providing managed services for InfluxDB, ELK, and Sensu}
  ...
  \end{itemize}
\item{Platforms: OpenStack, GCP, Kubernetes, Rancher}
\item{CICD: GitHub, GitLab, BitBucket, Concourse, Helm, Chef, ArgoCD}
\item{Languages: Golang, Bash, Ruby, Python, Cue}
